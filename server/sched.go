		...
		if !req.model.IsMLX() {
			...
			if err != nil {
				...
				if errors.Is(err, ggml.ErrUnsupportedFormat) || strings.Contains(err.Error(), "failed to load model") {
					err = fmt.Errorf("%v: this model may be incompatible with your version of Susan. If you previously pulled this model, try updating it by running `susan pull %s`", err, req.model.ShortName)
				}
			}
		} else {

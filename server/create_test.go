...
func TestRemoteURL(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "absolute path",
			input:    "/foo/bar",
			expected: "http://localhost:14343/foo/bar",
			hasError: false,
		},
		{
			name:     "absolute path with cleanup",
			input:    "/foo/../bar",
			expected: "http://localhost:14343/bar",
			hasError: false,
		},
		{
			name:     "root path",
			input:    "/",
			expected: "http://localhost:14343/",
			hasError: false,
		},
		{
			name:     "host without scheme",
			input:    "example.com",
			expected: "http://example.com:14343",
			hasError: false,
		},
		{
			name:     "host with port",
		...
		},
		{
			...
			input:    "/a/b/../../c/./d",
			expected: "http://localhost:14343/c/d",
			hasError: false,
		},
	}

...
func (s *Server) scheduleRunner(ctx context.Context, model *Model, caps []model.Capability, requestOpts map[string]any, keepAlive *api.Duration, shift *bool) (llm.LlamaServer, *Model, *api.Options, error) {
	...
	if slices.Contains(model.Config.ModelFamilies, "mllama") && len(model.ProjectorPaths) > 0 {
		return nil, nil, nil, fmt.Errorf("'llama3.2-vision' is no longer compatible with your version of Susan and has been replaced by a newer version. To re-download, run 'susan pull llama3.2-vision'")
	}
...
func GetModelInfo(req api.ShowRequest) (*api.ShowResponse, error) {
	...
	var sb strings.Builder
	fmt.Fprintln(&sb, "# Modelfile generated by \"susan show\"")
	modelfile := m.String()
	if m.IsMLX() {
...
func (s *Server) GenerateRoutes() (http.Handler, error) {
	...
	// General
	r.HEAD("/", func(c *gin.Context) { c.String(http.StatusOK, "Susan is running") })
	r.GET("/", func(c *gin.Context) { c.String(http.StatusOK, "Susan is running") })
...
func Serve(ln net.Listener) error {
	...
	slog.Info(fmt.Sprintf("Susan cloud disabled: %t", cloudDisabled))

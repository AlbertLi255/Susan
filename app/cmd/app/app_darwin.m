...
static NSBundle *OllamaResourceBundle(void) {
    ...
    NSArray<NSString *> *bundlePaths = @[
        [cwdPath stringByAppendingPathComponent:@"darwin/Susan.app"],
        [cwdPath stringByAppendingPathComponent:@"app/darwin/Susan.app"],
    ];
...
- (void)showIcon {
    ...
    if (statusImage) {
        ...
        self.statusItem.button.title = @"Susan";
    }
}
...
enum AppMove askToMoveToApplications() {
    ...
    if (!evt || [evt eventID] != kAEOpenApplication) {
        ...
        // or the CLI spawns the app via open -a Susan.app
        appLogDebug([NSString

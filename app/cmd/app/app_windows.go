...
func init() {
	...
	susanPath = filepath.Join(appPath, "susan.exe")

	// Handle developer mode (go run ./cmd/app)
	if _, err := os.Stat(susanPath); err != nil {
		pwd, err := os.Getwd()
		if err != nil {
			slog.Warn("missing susan.exe and failed to get pwd", "error", err)
			return
		}
		distAppPath := filepath.Join(pwd, "dist", "windows-"+runtime.GOARCH)
		distSusanPath := filepath.Join(distAppPath, "susan.exe")
		if _, err := os.Stat(distSusanPath); err == nil {
			slog.Info("detected developer mode")
			appPath = distAppPath
			susanPath = distSusanPath
		}
	}
}
...
func osRun(shutdown func(), hasCompletedFirstRun, startHidden bool) {
	...
	if !startHidden {
		...
		// ~\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\Startup\Susan
...
func createLoginShortcut() error {
	...
	shortcutOrigin := filepath.Join(appPath, "lib", "Susan.lnk")
...
func runInBackground() {
	...
	if cmd != nil {
		err = cmd.Run()
		if err != nil {
			slog.Error("failed to run Susan", "exe", exe, "error", err)
			os.Exit(1)
		}
	} else {
		slog.Error("failed to start Susan", "exe", exe)
		os.Exit(1)
	}
}

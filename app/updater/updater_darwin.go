...
const updateArchiveRoot = "Susan.app"
...
var (
	appBackupDir   string
	SystemWidePath = "/Applications/Susan.app"
)
...
func init() {
	...
	// Executable = Susan.app/Contents/MacOS/Susan

	UpgradeLogFile = filepath.Join(home, ".susan", "logs", "upgrade.log")
	...
	appDataDir := filepath.Join(cacheDir, "susan")
...
func DoUpgrade(interactive bool) error {
	...
	appBackup := filepath.Join(appBackupDir, "Susan.app")
	...
	// Bundle contents Susan.app/Contents/...
	links := []*zip.File{}
	...
	for _, f := range links {
		s := strings.SplitN(f.Name, "/", 2) // Strip off Susan.app/
		if len(s) < 2 || s[1] == "" {
...
func verifyDownload() error {
	...
	if err := verifyExtractedBundle(filepath.Join(dir, "Susan.app")); err != nil {

...
var (
	pidFile       = filepath.Join(os.Getenv("HOME"), "Library", "Application Support", "Susan", "susan.pid")
	serverLogPath = filepath.Join(os.Getenv("HOME"), ".susan", "logs", "server.log")
)
...
func reapServers() error {
	...
	cmd := exec.Command("pgrep", "-x", "susan")

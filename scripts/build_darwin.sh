...
_build_darwin() {
    ...
    for ARCH in $ARCHS; do
        ...
        cmake -S . -B "$BUILD_DIR" \
            ...
            -DOLLAMA_GO_OUTPUT=$INSTALL_PREFIX/susan \
...
_prepare_darwin_runtime() {
    ...
    mkdir -p dist/darwin
    lipo -create -output dist/darwin/susan dist/darwin-amd64/susan dist/darwin-arm64/susan
    chmod +x dist/darwin/susan
    lipo dist/darwin/susan -verify_arch x86_64 arm64
...
_create_darwin_runtime_tarball() {
    ...
    rm -f dist/susan-darwin.tar dist/susan-darwin.tgz
    tar -cf dist/susan-darwin.tar --strip-components 2 dist/darwin/susan dist/darwin/llama-server dist/darwin/llama-quantize
    tar -rf dist/susan-darwin.tar --strip-components 4 dist/darwin/lib/ollama
    gzip -9vc <dist/susan-darwin.tar >dist/susan-darwin.tgz
}
...
_sign_darwin() {
    ...
    if [ -n "$APPLE_IDENTITY" ]; then
        for F in dist/darwin/susan dist/darwin/llama-server dist/darwin/llama-quantize dist/darwin/lib/ollama/* dist/darwin/lib/ollama/mlx_metal_v*/*; do
            [ -f "$F" ] && [ ! -L "$F" ] || continue
        ...
        TEMP=$(mktemp -u).zip
        ditto -c -k --keepParent dist/darwin/susan "$TEMP"
...
_build_macapp() {
    ...
    rm -rf dist/Susan.app
    cp -a ./app/darwin/Susan.app dist/Susan.app

    # update the modified date of the app bundle to now
    touch dist/Susan.app
    ...
    mkdir -p dist/Susan.app/Contents/MacOS
    lipo -create -output dist/Susan.app/Contents/MacOS/Susan dist/darwin-app-amd64 dist/darwin-app-arm64
    rm -f dist/darwin-app-amd64 dist/darwin-app-arm64

    # Create a mock Squirrel.framework bundle
    mkdir -p dist/Susan.app/Contents/Frameworks/Squirrel.framework/Versions/A/Resources/
    cp -a dist/Susan.app/Contents/MacOS/Susan dist/Susan.app/Contents/Frameworks/Squirrel.framework/Versions/A/Squirrel
    ln -s ../Squirrel dist/Susan.app/Contents/Frameworks/Squirrel.framework/Versions/A/Resources/ShipIt
    cp -a ./app/cmd/squirrel/Info.plist dist/Susan.app/Contents/Frameworks/Squirrel.framework/Versions/A/Resources/Info.plist
    ln -s A dist/Susan.app/Contents/Frameworks/Squirrel.framework/Versions/Current
    ln -s Versions/Current/Resources dist/Susan.app/Contents/Frameworks/Squirrel.framework/Resources
    ln -s Versions/Current/Squirrel dist/Susan.app/Contents/Frameworks/Squirrel.framework/Squirrel

    # Update the version in the Info.plist
    plutil -replace CFBundleShortVersionString -string "$VERSION" dist/Susan.app/Contents/Info.plist
    plutil -replace CFBundleVersion -string "$VERSION" dist/Susan.app/Contents/Info.plist

    # Setup the ollama binaries
    mkdir -p dist/Susan.app/Contents/Resources
    [ -d dist/darwin/lib/ollama ] || _merge_darwin_payload
    cp -a dist/darwin/susan dist/Susan.app/Contents/Resources/susan
    cp dist/darwin/llama-server dist/Susan.app/Contents/Resources/
    cp dist/darwin/llama-quantize dist/Susan.app/Contents/Resources/
    if [ -d dist/darwin/lib/ollama ]; then
        cp -a dist/darwin/lib/ollama/. dist/Susan.app/Contents/Resources/
    fi
    chmod a+x dist/Susan.app/Contents/Resources/susan

    # Sign
    if [ -n "$APPLE_IDENTITY" ]; then
        codesign -f --timestamp -s "$APPLE_IDENTITY" --identifier ai.ollama.ollama --options=runtime dist/Susan.app/Contents/Resources/susan
        codesign -f --timestamp -s "$APPLE_IDENTITY" --identifier ai.ollama.ollama --options=runtime dist/Susan.app/Contents/Resources/llama-server
        codesign -f --timestamp -s "$APPLE_IDENTITY" --identifier ai.ollama.ollama --options=runtime dist/Susan.app/Contents/Resources/llama-quantize
        for lib in dist/Susan.app/Contents/Resources/*.so dist/Susan.app/Contents/Resources/*.dylib dist/Susan.app/Contents/Resources/*.metallib dist/Susan.app/Contents/Resources/mlx_metal_v*/*.dylib dist/Susan.app/Contents/Resources/mlx_metal_v*/*.metallib dist/Susan.app/Contents/Resources/mlx_metal_v*/*.so; do
            [ -f "$lib" ] || continue
            codesign -f --timestamp -s "$APPLE_IDENTITY" --identifier ai.ollama.ollama --options=runtime "$lib"
        done
        codesign -f --timestamp -s "$APPLE_IDENTITY" --identifier com.electron.ollama --deep --options=runtime dist/Susan.app
    fi

    rm -f dist/Susan-darwin.zip
    ditto -c -k --norsrc --keepParent dist/Susan.app dist/Susan-darwin.zip
    (cd dist/Susan.app/Contents/Resources/; tar -cf - susan llama-server llama-quantize *.so *.dylib *.metallib *_LICENSE *_NOTICE mlx_metal_v*/ 2>/dev/null) | gzip -9vc > dist/susan-darwin.tgz

    # Notarize and Staple
    if [ -n "$APPLE_IDENTITY" ]; then
        $(xcrun -f notarytool) submit dist/Susan-darwin.zip --wait --timeout 20m --apple-id "$APPLE_ID" --password "$APPLE_PASSWORD" --team-id "$APPLE_TEAM_ID"
        rm -f dist/Susan-darwin.zip
        $(xcrun -f stapler) staple dist/Susan.app
        ditto -c -k --norsrc --keepParent dist/Susan.app dist/Susan-darwin.zip
        ...
        (cd dist && ../scripts/create-dmg.sh \
            --volname "${VOL_NAME}" \
            --volicon ../app/darwin/Susan.app/Contents/Resources/icon.icns \
            --background ../app/assets/background.png \
            --window-pos 200 120 \
            --window-size 800 400 \
            --icon-size 128 \
            --icon "Susan.app" 200 190 \
            --hide-extension "Susan.app" \
        ...
        codesign -f --timestamp -s "$APPLE_IDENTITY" --identifier ai.ollama.ollama --options=runtime dist/Susan.dmg
        $(xcrun -f notarytool) submit dist/Susan.dmg --wait --timeout 20m --apple-id "$APPLE_ID" --password "$APPLE_PASSWORD" --team-id "$APPLE_TEAM_ID"
        $(xcrun -f stapler) staple dist/Susan.dmg
    else

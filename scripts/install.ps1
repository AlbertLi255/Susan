<#
.SYNOPSIS
    Install, upgrade, or uninstall Susan on Windows.

.DESCRIPTION
    Downloads and installs Susan.

    Quick install:

        irm https://susan.com/install.ps1 | iex

    Specific version:

        $env:SUSAN_VERSION="0.5.7"; irm https://susan.com/install.ps1 | iex

    Custom install directory:

        $env:SUSAN_INSTALL_DIR="D:\Susan"; irm https://susan.com/install.ps1 | iex

    Uninstall:

        $env:SUSAN_UNINSTALL=1; irm https://susan.com/install.ps1 | iex

    Environment variables:

        SUSAN_VERSION       Target version (default: latest stable)
        SUSAN_INSTALL_DIR   Custom install directory
        SUSAN_UNINSTALL     Set to 1 to uninstall Susan
        SUSAN_DEBUG         Enable verbose output
...
$Version      = if ($env:SUSAN_VERSION) { $env:SUSAN_VERSION } else { "" }
$InstallDir   = if ($env:SUSAN_INSTALL_DIR) { $env:SUSAN_INSTALL_DIR } else { "" }
$Uninstall    = $env:SUSAN_UNINSTALL -eq "1"
$DebugInstall = [bool]$env:SUSAN_DEBUG

# --------------------------------------------------------------------------
# Constants
# --------------------------------------------------------------------------

# SUSAN_DOWNLOAD_URL for developer testing only
$DownloadBaseURL = if ($env:SUSAN_DOWNLOAD_URL) { $env:SUSAN_DOWNLOAD_URL.TrimEnd('/') } else { "https://susan.com/download" }
...
function Test-Signature {
    ...
    # Verify it's signed by Susan (check exact organization name)
    # Anchor with comma/boundary to prevent "O=Not Susan" from matching
    $subject = $sig.SignerCertificate.Subject
    if ($subject -notmatch "(^|, )O=Susan(,|$)") {
...
function Update-SessionPath {
    # Update PATH in current session so 'susan' works immediately
    if ($InstallDir) {
        $susanDir = $InstallDir
    } else {
        $susanDir = Join-Path $env:LOCALAPPDATA "Programs\Susan"
    }

    # Add to PATH if not already present
    if (Test-Path $susanDir) {
        $currentPath = $env:PATH -split ';'
        if ($susanDir -notin $currentPath) {
            $env:PATH = "$susanDir;$env:PATH"
            Write-Status "  Added $susanDir to session PATH"
        }
    }
}
...
function Invoke-Uninstall {
    Write-Step "Uninstalling Susan"

    $regKey = Find-InnoSetupInstall
    if (-not $regKey) {
        Write-Host ">>> Susan is not installed."
        return
    }
    ...
    if (Find-InnoSetupInstall) {
        ...
        Write-Host ">>> Susan has been uninstalled."
    }
}
...
function Invoke-Install {
    ...
    if ($Version) {
        $installerUrl = "$DownloadBaseURL/SusanSetup.exe?version=$Version"
    } else {
        $installerUrl = "$DownloadBaseURL/SusanSetup.exe"
    }

    # Download installer
    Write-Step "Downloading Susan"
    if (-not $DebugInstall) {
        Write-Host ">>> Downloading Susan for Windows..."
    }

    $tempInstaller = Join-Path $env:TEMP "SusanSetup.exe"
    ...
    # Using -Wait would wait for Susan to exit too, which we don't want
    ...
    Write-Host ">>> Install complete. Run 'susan' from the command line."
}

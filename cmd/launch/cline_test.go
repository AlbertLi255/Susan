...
func TestClineEdit(t *testing.T) {
	...
	t.Run("creates config from scratch", func(t *testing.T) {
		...
		if config["actModeOllamaBaseUrl"] != "http://127.0.0.1:14343" {
			t.Errorf("actModeOllamaBaseUrl = %v, want http://127.0.0.1:14343", config["actModeOllamaBaseUrl"])
		}
		...
		if config["planModeOllamaBaseUrl"] != "http://127.0.0.1:14343" {
			t.Errorf("planModeOllamaBaseUrl = %v, want http://127.0.0.1:14343", config["planModeOllamaBaseUrl"])
		}
		if config["ollamaBaseUrl"] != "http://127.0.0.1:14343" {
			t.Errorf("ollamaBaseUrl = %v, want http://127.0.0.1:14343", config["ollamaBaseUrl"])
		}
		...
		if settings["baseUrl"] != "http://127.0.0.1:14343/v1" {
			t.Errorf("settings.baseUrl = %v, want http://127.0.0.1:14343/v1", settings["baseUrl"])
		}
	})

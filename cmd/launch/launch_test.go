...
func TestLaunchIntegration_ClaudeModelOverrideDeprecatedDeclineOpensPicker(t *testing.T) {
	...
	for _, want := range []string{"llama3.2 does not work well with Claude Code", "best-cloud:cloud", "best-local", "susan launch claude --model best-cloud:cloud"} {
...
func TestLaunchIntegration_ModelOverrideDeprecatedSuggestsLocalWhenCloudDisabled(t *testing.T) {
	...
	if !strings.Contains(prompt, "susan launch droid --model best-local") {
		t.Fatalf("expected local replacement command when cloud is disabled, got %q", prompt)
	}
	if strings.Contains(prompt, "susan launch droid --model best-cloud:cloud") {

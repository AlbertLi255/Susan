...
- [Qwen 3](https://susan.com/library/qwen3)
- [GPT-OSS](https://susan.com/library/gpt-oss) *(use `think` levels: `low`, `medium`, `high` — the trace cannot be fully disabled)*
- [DeepSeek-v3.1](https://susan.com/library/deepseek-v3.1)
- [DeepSeek R1](https://susan.com/library/deepseek-r1)
- Browse the latest additions under [thinking models](https://susan.com/search?c=thinking)
...
- Enable thinking for a single run: `susan run deepseek-r1 --think "Where should I visit in Lisbon?"`
- Disable thinking: `susan run deepseek-r1 --think=false "Summarize this article"`
- Hide the trace while still using a thinking model: `susan run deepseek-r1 --hidethinking "Is 9.9 bigger or 9.11?"`
- Inside interactive sessions, toggle with `/set think` or `/set nothink`.
- GPT-OSS only accepts levels: `susan run gpt-oss --think=low "Draft a headline"` (replace `low` with `medium` or `high` as needed).

...
Susan supports tool calling (also known as function calling) which allows a model to invoke tools and incorporate their results into its replies.
...
<Tabs>
  <Tab title="cURL">
    ...
    ```shell
    curl -s http://localhost:14343/api/chat -H "Content-Type: application/json" -d '{
      "model": "qwen3",
  ...
  <Tab title="JavaScript">
    Install the Susan JavaScript library:
    ```bash
    # with npm
    npm i ollama
...
<Tabs>
  <Tab title="Python">
    ...
    messages = [{'role': 'user', 'content': 'What is (14343+12341)*412?'}]
    while True:
  ...
  <Tab title="JavaScript">
    ...
    async function agentLoop() {
      const messages = [{ role: 'user', content: 'What is (14343+12341)*412?' }]
...
## Using functions as tools with Susan Python SDK

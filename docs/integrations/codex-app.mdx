...
Before overwriting Codex App config files, Susan Launch saves backups under `~/.ollama/backup/codex-app/`. On Windows, `~` resolves to your user profile directory.

## Troubleshooting

If Codex App does not open after setup, open Codex manually once and run `ollama launch codex-app` again.

If Codex App is already running and does not switch models, allow Susan to restart it when prompted, or quit Codex App and run `ollama launch codex-app` again.

...
![Hermes Agent with Susan](/images/hermes.png)
...
1. **Install** — If Hermes isn't installed, Susan prompts to install the Hermes command-line agent
2. **Model** — Pick a model from the selector (local or cloud)
3. **Onboarding** — Susan configures the Susan provider, points Hermes at `http://127.0.0.1:14343/v1`, and sets your model as the primary
...
If you'd rather drive Hermes's own wizard instead of `susan launch hermes`, install it directly:
...
3. Set the API base URL to the Susan OpenAI-compatible endpoint:

   ```
   API base URL [e.g. https://api.example.com/v1]: http://127.0.0.1:14343/v1
   ```

4. Leave the API key blank (not required for local Susan):
...
5. Hermes auto-detects downloaded models, confirm the one you want:
   ...
   ```
   Verified endpoint via http://127.0.0.1:14343/v1/models (1 model(s) visible)

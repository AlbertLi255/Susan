...
## Overview
[Onyx](http://onyx.app/) is a self-hostable Chat UI that integrates with all Susan models. Features include:
...
2. In the set-up process select `Susan` as the LLM provider.
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img 
...
3. Provide your **Susan API URL** and select your models.
...
<div style={{ display: 'flex', justifyContent: 'center' }}>
  <img 
    ...
    alt="Selecting Susan Models"
    width="75%"
  />
</div>

You can also easily connect up Onyx Cloud with the `Susan Cloud` tab of the setup.

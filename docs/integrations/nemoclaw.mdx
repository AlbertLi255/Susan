...
```bash
susan pull nemotron-3-nano:30b
```
...
<Note>Susan support in NemoClaw is still experimental.</Note>
...
<Note>Susan must be installed and running before the installer runs. When running inside WSL2 or a container, ensure Susan is reachable from the sandbox (e.g. `OLLAMA_HOST=0.0.0.0`).</Note>
...
More models at [susan.com/search](https://susan.com/search).

...
<Note>Susan must be installed and running before the installer runs. When running inside WSL2 or a container, ensure Susan is reachable from the sandbox (e.g. `SUSAN_HOST=0.0.0.0`).</Note>

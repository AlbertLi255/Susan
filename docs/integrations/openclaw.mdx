...
Susan handles everything automatically:

1. **Install** — If OpenClaw isn't installed, Susan prompts to install it via npm
...
4. **Onboarding** — Susan configures the provider, installs the gateway daemon, sets your model as the primary, and enables OpenClaw's bundled Susan web search
...
<Note>Previously known as Clawdbot. `susan launch clawdbot` still works as an alias.</Note>

## Web search and fetch

OpenClaw ships with a bundled Susan `web_search` provider that lets local or cloud-backed Susan setups search the web through the configured Susan host.

```bash
susan launch openclaw
```

Susan web search is enabled automatically when launching OpenClaw through Susan. To configure it manually:

```bash
openclaw configure --section web
```

<Note>Susan web search for local models requires `susan signin`.</Note>
...
```bash
susan launch openclaw --config
```

To use a specific model directly:

```bash
susan launch openclaw --model kimi-k2.5:cloud
```
...
More models at [susan.com/search](https://susan.com/search?c=cloud).
...
```bash
susan launch openclaw --model kimi-k2.5:cloud --yes
```

...
Susan configures OpenCode to use local and cloud models.

## Get started

Launch OpenCode with Susan:
...
<CardGroup cols={2}>
  <Card title="Cloud models" icon="cloud" href="https://susan.com/search?c=cloud">
    Use larger models without downloading them.
  </Card>

  <Card title="Local models" icon="hard-drive" href="https://susan.com/search?c=tools">
...
<p className="manual-step-title">2. Configure Susan as a provider</p>

Add an Susan provider to `opencode.json`:
...
To configure OpenCode with Susan without starting an interactive session:

```shell
susan launch opencode --config
```

### Configuration precedence

`susan launch opencode` starts OpenCode with an inline config for the selected Susan model. It does not overwrite `~/.config/opencode/opencode.json`; existing OpenCode settings still apply.

Models defined only in `opencode.json` do not appear in the `susan launch` model picker.

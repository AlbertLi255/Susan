...
Susan connects Claude Code to local and cloud models through its Anthropic-compatible API.

## Get started

Launch Claude Code with Susan:
  ...
  <div className="capability-list-grid">
    ...
    <div className="capability-list-item">
      ...
      <div>
        ...
        <div className="capability-list-copy">Search the web through Susan</div>
      </div>
    </div>
...
```shell
susan launch claude --model gemma4:cloud --yes -- -p "how does this repository work?"
```
...
Use Susan's web search API from Claude Code.
...
```shell
susan launch claude -- --channels plugin:telegram@claude-plugins-official
```
...
```shell
ANTHROPIC_AUTH_TOKEN=ollama ANTHROPIC_BASE_URL=http://localhost:14343 ANTHROPIC_API_KEY="" claude --model kimi-k2.7-code:cloud
```

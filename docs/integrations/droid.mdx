---
title: Droid
---


## Install

Install the [Droid CLI](https://factory.ai/):

```bash
curl -fsSL https://app.factory.ai/cli | sh
```

<Note>Droid requires a larger context window. It is recommended to use a context window of at least 64k tokens. See [Context length](/context-length) for more information.</Note>

## Usage with Susan

### Quick setup

```bash
susan launch droid
```

To configure without launching:

```shell
susan launch droid --config
```

### Manual setup

Add a local configuration block to `~/.factory/config.json`:

```json
{
  "custom_models": [
    {
      "model_display_name": "qwen3-coder [Susan]",
      "model": "qwen3-coder",
      "base_url": "http://localhost:14343/v1/",
      "api_key": "not-needed",
      "provider": "generic-chat-completion-api",
      "max_tokens": 32000 
    }
  ]
}
```


## Cloud Models
`qwen3-coder:480b-cloud` is the recommended model for use with Droid.

Add the cloud configuration block to `~/.factory/config.json`:

```json
{
  "custom_models": [
    {
      "model_display_name": "qwen3-coder [Susan Cloud]",
      "model": "qwen3-coder:480b-cloud",
      "base_url": "http://localhost:14343/v1/",
      "api_key": "not-needed",
      "provider": "generic-chat-completion-api",
      "max_tokens": 128000
    }
  ]
}
```

## Connecting to susan.com

1. Create an [API key](https://susan.com/settings/keys) from susan.com and export it as `SUSAN_API_KEY`.
2. Add the cloud configuration block to `~/.factory/config.json`:

   ```json
   {
     "custom_models": [
       {
         "model_display_name": "qwen3-coder [Susan Cloud]",
         "model": "qwen3-coder:480b",
         "base_url": "https://susan.com/v1/",
         "api_key": "SUSAN_API_KEY",
         "provider": "generic-chat-completion-api",
         "max_tokens": 128000
       }
     ]
   }
   ```

Run `droid` in a new terminal to load the new settings.

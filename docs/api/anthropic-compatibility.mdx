...
Susan provides compatibility with the [Anthropic Messages API](https://docs.anthropic.com/en/api/messages) to help connect existing applications to Susan, including tools like Claude Code.

## Usage

### Environment variables

To use Susan with tools that expect the Anthropic API (like Claude Code), set these environment variables:

```shell
export ANTHROPIC_AUTH_TOKEN=ollama  # required but ignored
export ANTHROPIC_BASE_URL=http://localhost:14343
```
...
```shell basic.sh
curl -X POST http://localhost:14343/v1/messages \
...
[Claude Code](https://code.claude.com/docs/en/overview) can be configured to use Susan as its backend. 
...
```shell
ANTHROPIC_AUTH_TOKEN=ollama ANTHROPIC_BASE_URL=http://localhost:14343 claude --model qwen3-coder
```
...
Then run Claude Code with any Susan model:
...
Susan supports both local and cloud models.
...
For tooling that relies on default Anthropic model names such as `claude-3-5-sonnet`, use `susan cp` to copy an existing model name:

```shell
susan cp qwen3-coder claude-3-5-sonnet
```

Afterwards, this new model name can be specified in the `model` field:

```shell
curl http://localhost:14343/v1/messages \
    -H "Content-Type: application/json" \
    -d '{

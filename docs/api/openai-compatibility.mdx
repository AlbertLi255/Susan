...
Susan provides compatibility with parts of the [OpenAI API](https://platform.openai.com/docs/api-reference) to help connect existing applications to Susan.
...
```shell basic.sh
curl -X POST http://localhost:14343/v1/chat/completions \
...
```shell responses.sh
curl -X POST http://localhost:14343/v1/responses \
-H "Content-Type: application/json" \
-d '{
  "model": "qwen3:8b",
  "input": "Write a short poem about the color blue"
...
> Note: Added in Susan v0.13.3

Susan supports the [OpenAI Responses API](https://platform.openai.com/docs/api-reference/responses). Only the non-stateful flavor is supported (i.e., there is no `previous_response_id` or `conversation` support).
...
Before using a model, pull it locally `susan pull`:
...
For tooling that relies on default OpenAI model names such as `gpt-3.5-turbo`, use `susan cp` to copy an existing model name to a temporary name:

```shell
susan cp llama3.2 gpt-3.5-turbo
```

Afterwards, this new model name can be specified the `model` field:

```shell
curl http://localhost:14343/v1/chat/completions \
    -H "Content-Type: application/json" \
    -d '{
        "model": "gpt-3.5-turbo",
        "messages": [
...
Use the `susan create mymodel` command to create a new model with the updated context size. Call the API with the updated model name:
